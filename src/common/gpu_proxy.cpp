...
int readInputDimsJsonFile(const char *configJSON, map<string,int> &name_to_id_mapping, unordered_map<int, vector<uint64_t>> &InputDimMapping){
    cout << __func__ << ": reading JSON File: " << configJSON
    <<endl;
    Json::Value root;
    std::ifstream ifs;
    ifs.open(configJSON);
    ...
    for(unsigned int i=0; i< root["Models"].size(); i++){
        string name = root["Models"][i]["name"].asString();
        int id = root["Models"][i]["proxy_id"].asInt();
        name_to_id_mapping[name]=id;
        #ifdef DEBUG
        cout << __func__ <<": setted up " << name.c_str() << " as " << id
        <<endl;
        #endif
...
int readInputTypesJSONFile(const char * configJSON, unordered_map<int,string> &input_map, unordered_map<int,string> &output_map){
     cout << __func__ << ": reading JSON File: " << configJSON
    <<endl;
    Json::Value root;
    std::ifstream ifs;
    ...
    for(unsigned int i=0; i< root["Models"].size(); i++){
        ...
        cout << __func__ <<": setted up " << id << "'s input_type as " << input_map[id]
...
int sendRequest(int in_fd, int rid, int jobid, std::vector<int64_t> &dims){
    ...
    int dim = dims.size();
#ifdef DEBUG
    cout << "[sendRequest] in_fd: " << in_fd << endl;
    cout << "[sendRequest] input has" <<  dims.size() <<" dimensions" <<endl;
...
#ifdef DEBUG
    cout << "[sendRequest] in_fd: " << in_fd << endl;
    cout << "[sendRequest] input has" <<  input_tensor.sizes().size() <<" dimensions" <<endl;
...
string proxy_info_to_string(proxy_info* pPInfo){
    string ret_string="["+ to_string(pPInfo->dev_id) + "," + to_string(pPInfo->cap) + "," + to_string(pPInfo->dedup_num)+"]";
    return ret_string;
