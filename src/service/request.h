...
public:
  vector<at::Tensor> _inputTensors; // in case model requires N tensor for inference, we maintain a vector of tensors
  request(int rid, int fd, int bnum);
  ~request();
  void setupAppInstance(shared_ptr<AppInstance> task);
