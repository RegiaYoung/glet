...
torch::Tensor getRandomTensor(vector<int64_t> dims);
...
void sendBatchedResults(shared_ptr<batched_request> &brp, string reqname); // defined in proxy_thread.cpp 
void addtoModelQueue(string StrName, torch::Tensor &input, int cgid,shared_ptr<AppInstance> task); // defined in server_thread.cpp
