...
extern std::map<std::string, mutex*> ReqMtxVec;
                 ...
                 it != ServerState.ReqListHashTable.end(); it++ ){
                    if(!it->second.empty()){
                        ...
                        while(!it->second.empty()){
                            ...
                            shared_ptr<request> req = it->second.front();
                            // compare with SLO
                    ...
                    if(!it->second.empty()){
                        ...
                        string type;
                         ...
                         for (int i =0; i < ServerState.nGPUs; i++){
                                for(auto pinfo : ServerState.PerDevMPSInfo[i]){
                                    for (auto tinfo : ServerState.PerProxyTaskList[pinfo]){
