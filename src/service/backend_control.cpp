...
int BackendControl::receiveControl(std::vector<int> &recv_data){
	...
	buf=SOCKET_rxsize(_socket_fd);
	buf = ServerState.FrontDevIDToHostDevID[buf];
	recv_data.push_back(buf); 
	buf=SOCKET_rxsize(_socket_fd);
	// get proxy index
	recv_data.push_back(buf); 
...
int BackendControl::executeControl(std::vector<int> &params){
	...
	if (cmd == BACKEND_GET_MEM){
		for(auto pair : ServerState.FrontDevIDToHostDevID){
	...
	int state=3;
	#ifdef BACKEND_DEBUG
	cout << __func__ << ": received command: " << cmd << " for dev_id: " << dev_id << " part_num: " << part_num
	<<endl;
	#endif
	switch(cmd){
...
int BackendControl::startProxy(proxy_info *pPInfo){
	//pProxyCtrl->startProxy(pPInfo,ServerState.nGPUs,ServerState.nProxyPerGPU);
	pProxyCtrl->startProxy(pPInfo,ServerState.nHostGPUs,ServerState.nProxyPerGPU);
