...
vector<shared_ptr<request>>& batched_request::getRequests(){
          return taskRequests;
}
...
void batched_request::setStrName(string name){
    _StrReqName = name;
}

string batched_request::getStrName(){
    return _StrReqName;
}
...
void batched_request::addRequestInfo(shared_ptr<request> &t){
...
void batched_request::allocateOutput(torch::Tensor tensor){
    ...
    vector<torch::Tensor> splitted_tensors = tensor.split(/*split size*/1,/*dim*/0);
    for(vector<shared_ptr<request>>::iterator it = taskRequests.begin(); it != taskRequests.end(); it++ ){
#ifdef DEBUG
