...
int recvBatchedTensor(int socket_fd, proxy_info* pPInfo){
    ...
    // 4. receive data and make tensor 
        ...
        std::cout << __func__ << " sucessfully received batch size "<< batch_size << " input, sending input to proxy " << std::endl;
        std::cout << __func__ << " input dims: ";
        for(auto dim :input_tensor.sizes()){
            std::cout << dim << " ";
        }
        std::cout << endl;
        
        std::cout << "task " << jid << " with batch size " << batch_size << "sent to proxy [ " << pPInfo->dev_id << ", " << pPInfo->partition_num << "]"
        <<std::endl;
        #endif
        pPInfo->sendMtx->lock();
        #ifndef NO_NET
