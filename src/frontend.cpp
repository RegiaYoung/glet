...
po::variables_map ParseOpts(int argCount, char** argVar){
  ...
  desc.add_options()("help,h", "Produce help message")
    ...
    ("sim_config", po::value<string>()->default_value("sim-config.json"), "json file which holds configurations for scheduler" )
    ("mem_config", po::value<string>()->default_value("mem-config.json"), "json file which holds profiled memory usage of each model" )
    ("latency_profile", po::value<string>()->default_value("latency.csv"), "list of model and configuration of proxys" )
    ("init_rate", po::value<string>()->default_value("rates.csv"), "list of initial rate for each model" )
    ("backend_json", po::value<string>()->default_value("BackendList.json"), "list of backends" )
    ("device_config, dc", po::value<string>()->default_value("device-config.json"), "json file which holds profiled info of per-device performance" )
...
int receiveClients(int socketfd){
  listen(socketfd, 20);
  LOG(ERROR) << "Server is listening for requests on " << socketfd <<endl;
...
void initDropThread(po::variables_map &vm, SysMonitor &state){
    ...
    if(drop_ratio) {// if drop ratio is not 0, initiate thread
       ...
       initDropThread(&state.DROP_RATIO);
    }
}
...
int configureProxys(BackendDelegate *backend_del, int node_id, SysMonitor *SysState){
  #ifdef FRONTEND_DEBUG
  cout << __func__ << ": called for node_id : " << node_id
  << endl;
  #endif
  gScheduler.createProxyInfos(SysState,backend_del->getSizes(),node_id,backend_del->getNGPUs(),backend_del->getType());
	 ...
	 for(int i =0; i < ServerState.nGPUs; i++){
        for(auto pPInfo : ServerState.PerDevMPSInfo[i]){
            cout << __func__ << ": configured " << proxy_info_to_string(pPInfo)
            << endl;
       }
    }
	#endif
...
int main(int argc, char* argv[]) {
  ...
  configureAppSpec(vm["config_json"].as<string>(),vm["resource_dir"].as<string>(),ServerState);
  // just wait for a few seconds, not sure why it prevent seg-faults but it works
  usleep(1*1000*1000);
  cout << "AppSpec configuration complete!" << endl;
  std::unordered_map<int, std::vector<uint64_t>> dummy;
  readInputDimsJsonFile(vm["proxy_json"].as<string>().c_str(), ServerState.PerModeltoIDMapping,dummy);
  usleep(1*1000*1000);
  cout << "model id to name mapping complete!" << endl;
  ...
  if(connectBackendList(vm["backend_json"].as<string>())){
      cout<<"Failed to connect backends" << endl;
      exit(1);
  }
  cout << "==== CONNECTING BACKEND DONE! ==== " << endl; 
  ...
  gScheduler.setupLatencyModel(vm["latency_profile"].as<string>());
	gScheduler.setupInitTgtRate(vm["init_rate"].as<string>());
  map<string,string> param_files;
  param_files["model_list"]=vm["model_list"].as<string>();
  param_files["sim_config"]=vm["sim_config"].as<string>();
  param_files["mem_config"]=vm["mem_config"].as<string>();
  param_files["proxy_json"] = vm["proxy_json"].as<string>();
  param_files["device_config"]=vm["device_config"].as<string>();
  gScheduler.setupScheduler(&ServerState, param_files,vm["scheduler"].as<string>(), vm["resource_dir"].as<string>());
  cout << "==== CONFIGURING SCHEDULER DONE! ====" << endl; 


  //run scheduling with intial rate and configured proxys
  // setup threads
  initProxyThreads(ServerState.nGPUs);
  initOutputThreads(ServerState.nGPUs); 
  ...
  if(vm["scheduler"].as<string>() == "mps_static"){
    gScheduler.readAndBoot(&ServerState, vm["model_list"].as<string>());
  }
  else{
    gScheduler.oneshotScheduling(&ServerState);
  }

  cout << "==== INITIAL SCHEDULING DONE! ===="<< endl; 
  
   
  initServerThread(ServerState.nGPUs);
