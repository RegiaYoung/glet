...
int NetworkLimitChecker::setupPerTaskInputDimension(std::string json_file){
    ...
    for(unsigned int i=0; i< root["Models"].size(); i++){
        ...
        _task_id_to_required_bytes_per_request[id]=size;
        #ifdef SCHED_DEBUG
        cout << __func__ <<": setted up " << id << " as " <<  _task_id_to_required_bytes_per_request[id]
        <<endl;
        #endif
    }
    ifs.close();
...
double NetworkLimitChecker::getRequiredBandwidth(GPUPtr gpu){
    ...
    for(auto node_ptr : gpu->vNodeList){
        for(auto task_ptr : node_ptr->vTaskList){
                sum_of_req_bandwidth += double(task_ptr->throughput) * _task_id_to_required_bytes_per_request[task_ptr->id];
        }
    }
    // return in gbits
...
int NetworkLimitChecker::adjustBatchSizetoNetBW(TaskPtr &task_ptr, GPUPtr &to_be_inserted_gpu){
    ...
    double remain_bw = max(_LIMIT_GBITS - getRequiredBandwidth(to_be_inserted_gpu), 0.0);
    #ifdef SCHED_DEBUG
    cout << __func__ <<": called with remaining bw : " << remain_bw << endl;
    #endif
    if(remain_bw == 0.0)
        return EXIT_FAILURE;
    ...
    for(int i = curr_batch; i>0;i--){
        double request_required_throughput = ((i/double(curr_batch)) * task_ptr->throughput) * _task_id_to_required_bytes_per_request[task_ptr->id];
        ...
        if(request_required_throughput <= remain_bw){
            ...
            #ifdef SCHED_DEBUG
            cout << __func__ <<": changed from " << org_trpt << " to " << task_ptr->throughput
            <<endl;
            #endif
            return EXIT_SUCCESS;
        }
